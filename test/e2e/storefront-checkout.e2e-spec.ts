...
describe('Storefront Checkout (e2e)', () => {
  ...
  async function registerAndAddToCart(quantity = 1) {
    ...
    const auth = await registerAndVerify(app, { email, password: 'SuperSecret123!', firstName: 'Check', lastName: 'Out' });
  ...
  it('rejects checkout with an empty cart', async () => {
    ...
    const auth = await registerAndVerify(app, { email, password: 'SuperSecret123!', firstName: 'Empty', lastName: 'Cart' });
